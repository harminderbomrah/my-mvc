...
class RelationManager{
	...
	public static function create_relation($model1,$model2){
		if(!self::check_for_relation($model1,$model2)){
			self::check_and_create_table($model1,$model2);
			$model1 = self::tabelize($model1);
			$model2 = self::tabelize($model2);
			array_push(self::$relations,array($model1,$model2));
		}
	}

	private static function check_for_relation($model1,$model2){
	...
	private static function check_and_create_table($model1, $model2){
		$db = new dbConnect();
	...
	public static function get_relations_for_model($model){
		$model = self::tabelize($model);
		$temp_relations = array_filter(self::$relations,function($array) use($model){ return(in_array($model, $array)); });
		$relations = array();
