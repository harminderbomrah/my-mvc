...
.controller('caseList', ['$scope', '$timeout', '$log', '$location', '$window', '$modal', '$jsonData', 'ngProgress', function($scope, $timeout, $log, $location, $window, $modal, $jsonData, ngProgress) {
  ...
  $scope.action = {
    ...
    itemAction: function(undo) {
      ...
      ngProgress.start();
      $jsonData.postData('POST', '/admin/case/', {action: type, id: $scope.initial.selection}, function(data, status) {
        toastr.success(msg);
        $scope.action.deselect(undo);
        ngProgress.complete();
      }, function(data, status) {
        toastr.error('Oops! There is something wrong whit server');
        $log.warn('case [', value ,'] is wrong');
        ngProgress.reset();
      });
    }
  };
...
.controller('caseForm', ['$scope', '$log', '$window', '$location', '$jsonData', 'ngProgress', function($scope, $log, $window, $location, $jsonData, ngProgress) {
  ...
  $scope.action = {
    ...
    new: function(value, event) {
      ...
      if((value && event.type == "click") || (value && event.keyCode == 13)) {
        ...
        if(exists == undefined) {
          ...
          // 如果送出的值沒有重複則將資料送出
          ngProgress.start();
          $jsonData.postData('POST', '/admin/case/', {action: 'add', value: value}, function(data, status) {
            //需要取得新增後的ID值
    ...
    doneEditing: function(value, index) {
      if($scope.initial.edited != value) {
        ...
        ngProgress.start();
        $jsonData.postData('POST', '/admin/case/', {action: 'update', id: $scope.initial.id, value: value}, function(data, status) {
    ...
    remove: function(item, index) {
      ...
      modalInstance.result.then(function(replaceID) {
        ...
        ngProgress.start();
        $jsonData.postData('POST', '/admin/case/', data, function(data, status) {
          if(replaceID != undefined) {
