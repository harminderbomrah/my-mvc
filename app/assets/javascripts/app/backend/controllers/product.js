...
.controller('productList', ['$scope', '$timeout', '$log', '$location', '$window', '$modal', '$jsonData', 'ngProgress', function($scope, $timeout, $log, $location, $window, $modal, $jsonData, ngProgress) {

  // get list json data use $jsonData services
  $jsonData.getData('/public/product-for-list.json').then(function(data) {
  ...
  $scope.initial = {
    ...
    orderName: "name",    // 預設的排序參數
    reverse: false,       // 預設的排序方向
  ...
  $scope.$watch('initial.currentPage', function(page_no) {
    $location.path("admin/product/" + String(page_no));
    var leng = page_no - 1
  ...
  $scope.action = {
    ...
    deselect: function(undo) {
      angular.forEach($scope.initial.selection, function(element) {
        ...
        angular.forEach($scope.productList, function(element, index) {
          if(mainElement == element.id) {
            if($scope.initial.trash) {
              undo ? element.checked = element.trash = false : $scope.productList.splice(index, 1)
            } else {
              element.trash = true;
...
.controller('productForm', ['$scope', '$log', '$window', '$location', '$jsonData', 'ngProgress', function($scope, $log, $window, $location, $jsonData, ngProgress) {
