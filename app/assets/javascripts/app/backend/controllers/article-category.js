  ...
  $scope.action = {
    ...
    new: function(value, event) {
      ...
      if((value && event.type == "click") || (value && event.keyCode == 13)) {
        ...
        if(exists == undefined) {
          ...
          // 如果送出的值沒有重複則將資料送出
          ngProgress.start();
          $jsonData.postData('POST', '/admin/article/category/new', {action: 'add', value: value}, function(data, status) {
            //需要取得新增後的ID值
    ...
    doneEditing: function(value, index) {
      if($scope.initial.edited != value) {
        ...
        ngProgress.start();
        $jsonData.postData('POST', '/admin/article/category/edit', {action: 'update', id: $scope.initial.id, value: value}, function(data, status) {
