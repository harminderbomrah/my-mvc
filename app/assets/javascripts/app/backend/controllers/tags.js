...
.controller('tags', ['$scope', '$log', '$timeout', '$modal', '$jsonData', 'ngProgress', function($scope, $log, $timeout, $modal, $jsonData, ngProgress) {

  // Definition main tag controller scope initial
  $scope.initial = {
    edited: null,
    id: null,
    buffer: false
  ...
  $scope.action = {
    ...
    new: function(value, event) {
      ...
      if((value && event.type == "click") || (value && event.keyCode == 13)) {
        ...
        // 檢查送出的值是否有與原始的資料重複
        var exists = null;
        exists = $scope.tag.filter(function(obj) {
    ...
    edit: function(value, index, event) {
      ...
      // 設定該物件為編輯中
      $scope.tag[index].edited = true;

      // 將該物件的id值傳入 $scope.initial.id
      $scope.initial.id = $scope.tag[index].id;
    ...
    remove: function(item, index) {
      var modalInstance = $modal.open({
        ...
        resolve: {
          ...
          tags: function() {
            var newTag = [];
            angular.forEach($scope.tag, function(value, idx) {
              if(idx != index) {
                newTag.push(value);
              }
            });
...
var ModalTagsCtrl = function ($scope, $log, $modalInstance, msg, replace, tags) {
  $scope.config = {
    tags: tags,
    tag: null,
    replace: replace,
  ...
  $scope.delete = function () {
    if($scope.config.replace) {
      if($scope.config.tag == null) {
        $scope.config.error = true;
      } else {
        $modalInstance.close($scope.config.tag);
      }
    } else {
      $modalInstance.close();
    }
